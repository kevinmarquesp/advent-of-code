defmodule Puzzle.Core.LogicTest do
  ...
  describe "chunk_freq/2" do
    test "chunks a list with an increasing value based on a function" do
      list = [1, 2, 3, 5, 6, 8, 9, 10]
      by = fn x -> x end

      assert Logic.chunk_freq(list, by) == [[1, 2, 3], [5, 6], [8, 9, 10]]
    end

    test "works with a custom function" do
      list = [2, 4, 6, 7, 10]
      by = fn x -> div(x, 2) end

      assert Logic.chunk_freq(list, by) == [[2, 4, 6], [7], [10]]
    end
    ...
    test "handles an empty list" do
      ...
      assert Logic.chunk_freq(list, by) == []
    end
    ...
    test "handles a single element list" do
      ...
      assert Logic.chunk_freq(list, by) == [[42]]
    end

    test "handles a list with no sequential elements" do
      list = [1, 10, 20, 30]
      by = fn x -> x end

      assert Logic.chunk_freq(list, by) == [[1], [10], [20], [30]]
    end

    test "works with unsorted lists when sorted beforehand" do
      list = [3, 1, 4, 2]
      by = fn x -> x end

      assert Logic.chunk_freq(Enum.sort(list), by) == [[1, 2, 3, 4]]
    end
  end
